...
Stop scrolling menus. Get a smart meal pick, then find a nearby place.
...
- Account creation: No
- Ads: No
- Precise or approximate location permission: No
- On-device data: Meal history, preferences, and blacklist entries are stored locally in SQLite.
- External navigation: The app opens a user-selected search query in Google Maps or a browser. Google handles activity there under its own privacy policy.
- Data deletion: Uninstalling the app removes its local app data. Blacklisted meals can also be restored inside the app.

## Assets still required from a real device

- At least two phone screenshots from the signed APK/AAB build
- Recommended captures: the suggestion screen, filters, and the blacklist/settings flow
